...
export class PiSocketClient {
	 ...
	 */
	static async connect(socketPath: string, onEvent?: (event: SocketEvent) => void): Promise<PiSocketClient> {
...
function validateHello(line: string): Error | undefined {
	try {
		const hello = JSON.parse(line) as { type?: string; protocol?: string; version?: number };
		if (hello.type !== "hello" || hello.protocol !== "pi-rpc-socket") {
			return new Error(`not a pi RPC socket (got ${line.slice(0, 100)})`);
		}
		if (hello.version !== 1) {
			process.stderr.write(`pi-ctl: warning: pi socket protocol version ${hello.version}, expected 1\n`);
		}
		return undefined;
	} catch {
