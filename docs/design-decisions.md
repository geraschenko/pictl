...
- `agent.json` authority split:
  ...
  - Session facts (current session file, model, streaming state) are queried live over `pi.sock` whenever the agent is running. `agent.json` additionally records `sessions` — the history of session files this agent has hosted, most recent last. The holder maintains it by connecting to its own `pi.sock` as a client and appending on the `session_changed` broadcast event, which fires whenever the session is replaced (`/new`, `/resume`, `switch_session`). This event was added to pi's tee mode specifically for this: responses to `new_session`/`switch_session` go only to the issuing connection, TUI-initiated switches are otherwise invisible to socket clients, and polling `get_state` per broadcast event is a non-starter (events fire per streamed token). This history enables dormant-agent revival and agent↔session mapping.
  - `sessions` entry semantics: pi defers writing a new session file until the first assistant message, so a `session_changed` event may announce a path that does not exist on disk yet. The holder therefore appends the new session as *pending* and only marks it confirmed once an assistant message arrives in that session; pending entries supersede nothing. The event's `sessionFile` is also optional (pi can run without session persistence); in-memory sessions never enter the history. Revival resumes the most recent *confirmed* entry, tolerating ENOENT as a backstop by falling back to the next most recent entry whose file exists, or starting fresh if none.
