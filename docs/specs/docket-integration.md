...
- **Fallback-scope finding**: in Claude Code-style harnesses every Bash tool call is a fresh parent process, so the ppid-derived inbox is stable only *within* one call; multi-step team flows must run in a single shell invocation there. Recorded here as a known limit of case 2; a tmux-based identity/notify extension is being explored in a separate spec.
