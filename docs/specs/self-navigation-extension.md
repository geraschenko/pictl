# Handoff: `/navigate-tree` extension (agent self-navigation)
...
```
pictl prompt "/navigate-tree <targetId> --summarize --continue 'Resume implementing X using the summary above.'"
```

(Or via a thin `pictl navigate-tree …` wrapper that just sends that prompt — see
"pictl surface" below.)
...
void (async () => {
  try {
    // TDC: to start, we can have this use waitForIdle, with a TODO to change it when waitForSettled lands
...
- The command is accepted immediately (it returns before navigating); the agent's
  ...
  navigation completes. Accepted practice: after issuing `/navigate-tree`, do not
...
- The minimal version needs **no pictl change**: the agent calls
  `pictl prompt "/navigate-tree …"` directly.
...
It's a pi extension (`export default function (pi: ExtensionAPI) { pi.registerCommand("navigate-tree", { … }) }`,
...
- **Arg parsing** for `--continue` values containing spaces/quotes — make sure the
  command arg string (everything after `/navigate-tree `) is split so the
