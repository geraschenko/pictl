...
- A pi binary supporting `--rpc-socket` (the tee-mode fork: `geraschenko/pi`, branch `anton/pi-tee`; spec in that repo at `docs/pi-rpc-socket-mode.md`). Verify with `pi --rpc-socket /tmp/test.sock` in a terminal — it must run interactive pi and create the socket.
...
- `pi-ctl spawn [--cwd <dir>] [--id <id>] [-- <pi args...>]`
  - generates an agent id (short random) unless given;
...
- `pi-ctl _hold` (hidden): allocates a PTY (default 80×24), runs `pi --rpc-socket <dir>/pi.sock <pi args...>` in it with `PI_AGENT_ID=<id>` in the environment, pipes PTY output into an @xterm/headless instance, writes `agent.json` (holder pid, pi pid, cwd, session path once known, spawn args, created-at), reaps pi on exit, removes the sockets on clean shutdown. The `tty.sock` server may be stubbed until phase 2, but bind it now so the directory shape is final.
- `pi-ctl list` — readdir + liveness probe per agent (holder pid alive; `get_state` over `pi.sock` for status: idle/streaming/dead/stale). Human-readable table; `--json` for machines.
- `pi-ctl status <agent>` — one agent, more detail (model, session file, cwd, pids).
- `pi-ctl kill <agent>` — graceful: ask the holder to terminate pi (SIGTERM to pi, escalate after timeout), holder exits, sockets removed.
- `pi-ctl gc` — remove agent dirs whose holder is dead.
...
Verification pause: spawn an agent, `list` shows it running, `kill` removes it, `gc` cleans a simulated crash (kill -9 the holder).
...
- Agent addressing: positional `<agent-id>`; if `PI_WORKFLOW_DIR` is set or `--workflow <dir>` given, also accept role names resolved through that workflow's state file. (Resolution only — workflow dir layout is otherwise out of scope for v1; keep the resolver tiny and tolerant.)
...
1. **Type imports vs vendoring** (phase 1): can pi-ctl declare a dependency on the fork's coding-agent package (git dependency / local path / published fork build) to import `RpcCommand`/`RpcResponse`/`SessionEntry` types? If not, vendor the types in one module with the pi commit recorded.
