...
- Bug found and fixed: pi sends the post-hello `session_changed` typically in the same TCP chunk as `hello`, so registering an event listener *after* connect loses it. `PiSocketClient.connect`/`connectWithRetry` therefore accept the listener as an argument and register it before dispatching any record.
...
- Bug found and fixed: every suspend/kill of a running agent took exactly 5s. Not pi-side — pi handles SIGTERM as graceful shutdown (SIGINT is "abort turn") and exits in ~10ms. The cause was `terminatePi`'s un-cleared `Promise.race` escalation `setTimeout`: node exits on empty event loop *handles* (it does not track promises), so the losing timer kept the CLI alive until it fired. Both race timers (`terminatePi`, `waitQuiescent` with `--timeout`) are now cleared after the race; suspend/kill of an idle agent measure ~0.1s, and `--timeout` expiry still reports "still busy" at the configured bound.
