...
- Holder side: `tty.sock` serves the framed protocol: snapshot on connect (serialized from @xterm/headless via `@xterm/addon-serialize`, **including scrollback** — reviewing what happened is a primary reason to attach), then raw PTY output. Multiple simultaneous clients; resize is last-writer-wins, applied as PTY resize (node-pty `resize()`, which delivers SIGWINCH only when the size actually changes). Snapshot correctness requirements (see work log for why): (a) a parse barrier — `terminal.write()` parses asynchronously, so buffer the new client's output relay until a write-callback barrier fires, then snapshot, then flush; (b) append the current cursor-visibility sequence (`?25l`/`?25h`) to the snapshot — the serialize addon does not capture it and pi runs cursor-hidden. On holder shutdown, send `exit` and destroy attach connections (don't just close the listener). A client that stops reading buffers unboundedly in Node — accepted for v1 (single machine, human attachers); revisit with a high-water disconnect if it bites.
- `pi-ctl attach <agent>`: put the local terminal in raw mode, connect, send initial resize, render snapshot, proxy bytes both ways; forward local SIGWINCH as resize frames. Detach keybinding: `ctrl+]` (byte 0x1D; in raw mode received as a byte, not a signal), defined as a named constant in exactly one place — no magic strings — so it stays easy to change or make configurable later. Print a one-line hint ("attached to <id>; detach: ctrl+]") before entering raw mode. The terminal must be restored on every exit path: detach, agent death mid-attach (`exit` frame or socket close — report it), and unexpected errors.
- Attaching to a dormant agent is an error ("agent is dormant; run `pi-ctl resume`") with a TODO comment: phase 3 replaces this with transparent revival.
...
- Remaining phase 2 sequencing: `src/tty-protocol.ts` (framing + unit tests) → holder side → `pi-ctl attach` → manual verification pause.
...
5. **Trust dialog on headless spawn** (phase 1, found during implementation) — pi blocks on an interactive "Trust project folder?" prompt before binding the RPC socket if the cwd has `.pi` inputs and no stored trust decision, so a headless spawn into such a dir fails after the holder's 30s connect deadline. Workaround: `pi-ctl spawn -- --approve` (pi's session-only trust override). Should pi-ctl do anything more (own flag, detect-and-explain in the error, or just document)?
